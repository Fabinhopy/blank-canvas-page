...
import {
  MessageSquarePlus, Clock, CheckCircle2, HelpCircle, Lightbulb, AlertCircle,
  Send, Loader2, Inbox, MessageSquare, ImagePlus, X, FolderKanban,
} from 'lucide-react';
    ...
    try {
      ...
      if (attachmentFile) {
        ...
        const { error: uploadError } = await supabase.storage
          .from('documents').upload(filePath, attachmentFile, { cacheControl: '3600', upsert: false });
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input placeholder="Descreva brevemente..." value={subject} onChange={e => setSubject(e.target.value)} required />
                  </div>
                  <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground flex items-center gap-2">
                    <FolderKanban className="h-4 w-4" /> Referência do Projeto *
                  </label>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea placeholder="Descreva sua dúvida, sugestão ou problema em detalhes..." value={message} onChange={e => setMessage(e.target.value)} rows={4} required />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground">Anexar Imagem (opcional)</label>
                  {attachmentPreview ? (
                    <div className="relative inline-block">
                      <img src={attachmentPreview} alt="Preview" className="max-h-32 rounded-lg border object-contain" />
                      <Button type="button" variant="destructive" size="icon" className="absolute -top-2 -right-2 h-6 w-6" onClick={removeAttachment}>
                        <X className="h-3 w-3" />
                      </Button>
                    </div>
                  ) : (
                    <label className="flex items-center gap-2 cursor-pointer text-sm text-muted-foreground hover:text-foreground transition-colors border border-dashed rounded-lg p-3">
                      <ImagePlus className="h-5 w-5" /><span>Clique para selecionar uma imagem</span>
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-sm">
                {isAdmin ? 'Nenhum cliente enviou chamados por enquanto.' : 'Clique em "Novo Chamado" para enviar sua primeira solicitação.'}
              </p>
            </CardContent>
          </Card>
        ) : (
                  ...
                  <DialogContent className="max-w-lg">
                    <DialogHeader>
                      <DialogTitle className="flex items-center gap-2">
                        <CatIcon className="h-5 w-5 text-primary" />{ticket.subject}
                      </DialogTitle>
                    </DialogHeader>
                      ...
                      {ticket.admin_response && (
                        <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
                          <p className="text-xs font-semibold text-primary mb-2 flex items-center gap-1">
                            <CheckCircle2 className="h-3 w-3" /> Resposta da equipe
                          </p>
